...
fn compact_history_once(
    ...
    if let Some(runtime_state) = runtime_state.filter(|value| !value.trim().is_empty()) {
        compacted.push(ChatMessage::text("user", runtime_state));
    }
