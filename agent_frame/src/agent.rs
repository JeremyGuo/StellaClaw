...
fn compose_system_prompt(config: &AgentConfig, skills: &[SkillMetadata]) -> String {
    ...
    let mut parts = vec![
        ...
        "Remote execution policy: when a tool exposes a remote parameter and the task is on an SSH host, you MUST use that tool with remote set to an actual local SSH alias, normally one listed in ~/.ssh/config and surfaced in the current system prompt, instead of manually running ssh <host> inside a shell command. Do not invent remote aliases; use the provided alias list, a registered remote workpath host, or ask the user. Manual SSH through shell is rejected when a remote-capable tool can perform the work. Omit remote, set remote=\"\", or set remote=\"local\" for local work. For shell, the remote session starts at the registered remote workpath when one exists, otherwise the remote user's home directory.".to_string(),
        "Runtime-ID start tools create background state and can also wait for short jobs in the same call. For dsl_start, file_download_start, and image_start, prefer the default wait-until-complete behavior for short bounded work; use their explicit wait tools only when the job needs to continue across model rounds.".to_string(),
...
mod tests {
    ...
    fn compose_system_prompt_uses_configured_skills_metadata_snapshot() {
        ...
        assert!(prompt.contains("Manual SSH through shell is rejected"));
        assert!(prompt.contains("Runtime-ID start tools"));
        assert!(
            prompt
                .contains("prefer the default wait-until-complete behavior for short bounded work")
        );
