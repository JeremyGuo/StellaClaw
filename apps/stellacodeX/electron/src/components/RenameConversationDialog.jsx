...
import { displayConversationName, displayForegroundSessionName } from '../lib/api';
...
export function RenameSessionDialog({ open, conversation, session, saving, onOpenChange, onRename }) {
  const [nickname, setNickname] = useState('');
  ...
  const submit = (event) => {
    event.preventDefault();
    if (!conversation || !session || saving) return;
    onRename?.(conversation, session, nickname.trim());
  };
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-content rename-conversation-dialog">
          <div className="dialog-titlebar rename-dialog-titlebar">
            <div className="rename-dialog-heading">
              <span className="rename-dialog-mark" aria-hidden="true">
                {String(currentName || 'S').slice(0, 1).toUpperCase()}
              </span>
              <div>
                <Dialog.Title>重命名 Session</Dialog.Title>
                <Dialog.Description>修改左侧 Session 显示名称。</Dialog.Description>
              </div>
            </div>
            <Dialog.Close className="dialog-close" type="button">×</Dialog.Close>
          </div>
          <form className="dialog-form rename-dialog-form" onSubmit={submit}>
            <label className="form-field rename-dialog-field">
              ...
              <input
                ...
                value={nickname}
                onChange={(event) => setNickname(event.target.value)}
                placeholder={currentName || 'Session'}
              />
            </label>
            <div className="dialog-actions rename-dialog-actions">
              <Dialog.Close className="secondary-button" type="button">取消</Dialog.Close>
              <button className="primary-button" type="submit" disabled={saving || !conversation || !session || !nickname.trim()}>
                {saving ? '正在保存...' : '保存'}
              </button>
            </div>
          </form>
