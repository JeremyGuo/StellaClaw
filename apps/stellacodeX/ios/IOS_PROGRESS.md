# StellaCodeX iOS Progress

This file tracks the native iOS client implementation. The Electron client is the feature and protocol reference, not the visual design reference.

## Product Direction

- Build a first-class iOS client for Stellaclaw's Web channel.
- Prefer native iOS platform patterns: `NavigationSplitView`/`NavigationStack`, safe-area-aware composer, touch targets, and compact inspector affordances.
...
- Reuse Stellaclaw Web API semantics from the Electron client.
- Do not recreate the Electron UI literally.
- Keep iOS client UI under `apps/stellacodeX/ios/`; shared protocol notes can live under `apps/stellacodeX/shared/` when needed.
...
- Terminal and workspace browsing should feel native on iOS, not like embedded web panels.

## Feature Parity Reference

From `apps/stellacodeX/electron`:
...
- [x] Moved the client directory from `apple/` to `ios/` and made iOS the native-client focus.
...
```bash
xcodebuild \
  -project apps/stellacodeX/ios/StellaCodeX/StellaCodeX.xcodeproj \
  -scheme StellaCodeX \
