...
```text
apps/stellacodeX/
  android/   # Android native client; first implementation target.
  ios/       # iOS native client placeholder.
  macos/     # macOS native client placeholder.
...
Each platform directory owns its native project files and build tooling. Shared code should only live in `shared/` when it is genuinely platform-neutral, such as protocol documentation, generated API schema/types, common icons, or test fixtures. Do not recreate an Electron-style shared UI layer here.
...
1. Build the Android client skeleton.
