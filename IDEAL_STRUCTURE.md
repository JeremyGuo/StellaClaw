...
这一节专门回答：和当前项目相比，哪些地方已经接近，哪些地方还明显不一样。

### 14.1 已经比较接近的部分

当前代码里已经有几块和理想结构方向一致：

- 已经存在 `ConversationManager`
  - conversation settings、workspace 绑定、remote execution、attachment materialization 都明显偏 conversation-owned
- 已经存在 `SessionManager` 和 mailbox-backed `SessionActorRef`
  - 每个 session 已经有自己的 actor worker 线程和串行状态转换
- session transcript 已经是 session-owned durable 数据
  - `transcript.jsonl` 挂在 session root 下，而不是 channel 直接写
- background agent、cron、snapshot、subagent 这些能力已经存在
  - 说明 host-owned manager 的概念并不是从零开始
- 当前代码已经强调：
  - conversation 负责准备消息
  - session 负责等待、interrupt、yield、progress、turn commit

也就是说，当前项目不是“完全没有这套架构”，而是：
...
#### 1. 还是以同进程直接编排为主，不是明确的跨进程 IPC 架构
...
- `CronManager`
- `SnapshotManager`
- Web/Telegram/DingTalk channel

都主要被 `agent_host/src/server.rs` 直接持有和调度。

这意味着：

- 现在的主边界更像“同进程模块 + actor ref”
- 还不是文档目标里的 `Conversation Process/Thread <-> Session Process/Thread` 明确 JSON-RPC/UDS IPC

#### 2. `server.rs` 仍然承担了大量 orchestration 职责

当前项目里很多流程还直接发生在 `server.rs`：

- incoming message 处理
- foreground/background session 启动
- progress fan-out
- cron polling
- subagent/background job 编排
- transcript 相关读取/分发

这和理想目标的差异是：

- 理想结构希望 `Conversation` 成为更硬的 ingress/router actor
- `SessionActor` 成为更硬的 execution/state-machine actor
- Host service 更像 transport + manager 容器，而不是主流程编排中心

#### 3. `Conversation` 目前更像 durable manager，还不是“协议边界上的独立 actor”

当前 `ConversationManager` 已经有 durable state，但它更偏：

- 配置管理
- foreground actor reference 管理
- attachment storage 解析
- remote/local mount 解析

它还没有完全长成文档里那个角色：

- 对外是稳定的 `Conversation Actor`
- 对内通过 typed IPC 把 prepared message / host coordination / query 统一转给 session

#### 4. 当前 session actor 是同进程 actor worker，不是独立 session process

现在 `SessionManager` 里已经会为 session actor 起 worker thread，但它依然是：

- 同一个 `agent_host` 进程里的线程
- 通过 `SessionActorRef` 的 direct actor method / channel 调用

理想结构则进一步要求：

- session 运行在独立 session process
- `Conversation -> Session` 走显式 IPC protocol
- transport 和 actor state machine 解耦

#### 5. host-owned 协作能力已经存在，但协议边界还没有统一
...
- 由 server/runtime 直接调用 manager
- 或由 session/foreground runtime 直接编排 host 侧动作

理想目标则是：

- 统一成 `HostCoordinationRequested -> ResolveHostCoordination`
- 让“谁发起、谁执行、谁回填结果”这一层协议明确下来

#### 6. Web history 已经存在，但还不是抽象成统一的 session view query 协议
...
但这一层更多是 Host 直接提供的 Web API 面，而不是文档里抽象出来的：

- `SessionRequest::QuerySessionView`
- `SessionEvent::SessionViewResult`

也就是说，当前项目有这个能力，但协议抽象还没有收束。

#### 7. tool surface 已经是动态生成，但还没有被明确纳入 actor/IPC 设计

当前代码里已经有：

- remote mode 下移除 `remote`
- vision 能力决定 `images` 是否出现在 schema
- foreground/background/cron 工具集不同

但这些更多还体现在 runtime/tool builder 逻辑里，尚未和：

- session mode
- host coordination
- compaction gate
- query/view capability
...
- `ConversationManager + SessionManager + SessionActorRef + Server Orchestrator`

理想结构希望收敛到：

- `Conversation Actor + Session RPC Boundary + SessionActor Process + Host Coordination Protocol`

所以真正的重构重点不是“推倒重写功能”，而是把现有能力重新压到更明确的边界里。

## 15. 重构计划与进度追踪

这一节作为后续大重构的持续追踪区。每次阶段完成后，都应该回来更新这里。
...
| 阶段 | 名称 | 目标 | 当前状态 |
|---|---|---|---|
| `P0` | 文档收敛 | 把理想结构、消息类型、tool schema、query/view、compaction gate 先定义清楚 | `已完成` |
| `P1` | 协议收敛 | 在代码里先定义统一的 request/event/query/coordination 类型，不急着拆进程 | `进行中` |
| `P2` | Host orchestration 下沉 | 把 `server.rs` 中直接编排的 session/host 协作路径改成通过 Conversation/Session 边界流转 | `未开始` |
| `P3` | Session view query 收敛 | 把 Web/history/detail/live read path 收敛到 `QuerySessionView` 风格协议 | `未开始` |
| `P4` | Host coordination 收敛 | 把 subagent/background/cron/snapshot 等能力收敛到统一 coordination 协议 | `未开始` |
| `P5` | Session runtime/transport 解耦 | 把 session actor loop 和 transport/rpc boundary 分开 | `未开始` |
| `P6` | 进程拆分 | 把 session actor 从同进程 worker thread 演化成独立 session process | `未开始` |
| `P7` | 清理旧路径 | 删除旧的 direct actor orchestration / server-side bypass 路径 | `未开始` |

### 15.3 每阶段建议产物

| 阶段 | 建议产物 |
|---|---|
| `P0` | `IDEAL_STRUCTURE.md` 收敛到可施工级别。 |
| `P1` | Rust `enum/struct` 草图：`SessionRequest`、`SessionEvent`、`HostCoordinationKind`、`SessionViewQueryKind`。 |
| `P2` | `server.rs` 里核心路径改为先经过统一 Conversation/session 边界。 |
| `P3` | Web transcript/history/detail 改为统一走 session view query。 |
| `P4` | subagent/background/cron/snapshot 全部改为统一 coordination 往返。 |
| `P5` | Session transport thread / rpc boundary 抽离完成。 |
| `P6` | UDS/JSON-RPC 跨进程 session process 跑通。 |
| `P7` | 删除旧 API、旧旁路、旧耦合代码。 |

### 15.4 当前优先级建议

如果按风险最低、收益最高的顺序推进，建议先做：

1. `P0 文档收敛`
2. `P1 协议类型收敛`
3. `P4 Host coordination 收敛`
4. `P3 Session view query 收敛`
5. `P2 Host orchestration 下沉`
6. `P5 Session runtime/transport 解耦`
7. `P6 进程拆分`
8. `P7 清理旧路径`

这样做的原因是：

- 先把协议钉住，后面改调用路径才不会反复返工
- 先把 host coordination 和 query path 收束，能先减轻 `server.rs` 的耦合
- 真正的跨进程拆分应该尽量放在协议和边界稳定之后

### 15.5 当前进度快照

截至当前这版文档，已经完成或基本完成的内容：

- 线程/进程优先视图
- `Conversation` 与 `SessionActor` 的边界重定义
- `Control Path` / `Data Path` mailbox 语义
- interrupt ownership 收敛到 `SessionActor`
- host coordination 协议抽象
- session view query 协议抽象
- Web 实时路径 vs 历史读取路径区分
- tool schema 动态裁剪规则
- tool 的 `Immediate / Interruptible` 分类
- compaction / mode switch / gating failure 约束
- `P1` 的 Rust 协议草图
...
- 统一的 Rust 协议类型定义
- `server.rs` 旧编排路径的替换
- `QuerySessionView` 真正接入 Web/history 代码路径
- `HostCoordinationRequested` 真正接入 subagent/background/cron/snapshot
- session transport 与 process 拆分
...
这一节的目标不是直接定死最终代码，而是先把跨边界协议压成一套能开工的 Rust 类型草图。

### 16.1 设计约束

- 只有三类跨边界请求：
  - `mailbox write`
  - `host coordination resolve`
  - `session view query`
- 只有四类跨边界事件：
  - `turn lifecycle`
  - `interactive output`
  - `host coordination request`
  - `query result / rejection / crash`
- `Data Path` 和 `Control Path` 的分流应该在协议层就可见
- query 语义必须和 mailbox write 语义分开，避免“查历史”误入 mailbox

### 16.2 SessionRequest 草图
...
pub enum SessionRequest {
    MailboxWrite(SessionMailboxWrite),
    ResolveHostCoordination(HostCoordinationResolution),
...
为了避免后面代码里大量裸 `String`/`Uuid` 混用，建议在 `P1` 阶段就把核心 ID 包一层 newtype。
...
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct ToolRequestId(pub uuid::Uuid);
...
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct ActorMessageId(pub uuid::Uuid);
```

建议统一约束：

- envelope 上的时间戳都用 `Unix epoch milliseconds`
- 外部可见 query / request / event 都必须带稳定 id
- `TurnId` 和 `ConversationMessageId` 不能互相复用

### 16.2.2 各类 ID 的职责区分

为了避免后面实现时把不同层次的 ID 混在一起，这里把职责固定下来：

| ID | 层次 | 谁生成 | 用来干什么 |
|---|---|---|---|
| `RequestId` | transport envelope | 请求发起方 | 关联一次通用 IPC request envelope。 |
| `EventId` | transport envelope | 事件发出方 | 关联一次通用 IPC event envelope。 |
| `QueryId` | 只读查询 | `Conversation` 或 query 发起方 | 关联一次 `QuerySessionView -> SessionViewResult` 往返。 |
| `ToolRequestId` | host 协作事务 | `SessionActor` | 关联一次 `HostCoordinationRequested -> ResolveHostCoordination` 往返。 |
| `TurnId` | session 执行 | `SessionActor` | 标识一次 session turn / operation 的生命周期。 |
| `ConversationMessageId` | conversation ingress | `Conversation` | 标识一条进入 session 的 canonical user message。 |
| `ActorMessageId` | actor-to-actor ingress | actor message 发起方 | 标识 background/subagent/system relay 这类 actor message。 |

可以用一句话记：

- `RequestId / EventId` 是 transport 层
- `QueryId / ToolRequestId` 是协议子流程层
- `TurnId / ConversationMessageId / ActorMessageId` 是业务对象层

### 16.2.3 生成规则建议

| ID | 生成时机 | 稳定性要求 |
|---|---|---|
| `RequestId` | 每次发 IPC request 时 | 同一次 request 重试时可以复用，也可以包一层 retry metadata。 |
| `EventId` | 每次发 IPC event 时 | 必须唯一，便于 sink/debug 去重。 |
| `QueryId` | 每次发起 session view query 时 | 同一轮 query-response 往返中保持不变。 |
| `ToolRequestId` | `SessionActor` 决定发起一次 host coordination 时 | 从 request 发出到 result resolve 全程保持不变。 |
| `TurnId` | `SessionActor` 真正开始一个 turn/maintenance operation 时 | 一个 turn 不应复用旧 `TurnId`。 |
| `ConversationMessageId` | `Conversation` 完成 canonicalization 时 | 必须稳定持久化，后续 dedupe/consume 都依赖它。 |
| `ActorMessageId` | actor message 创建时 | 必须稳定，用于 actor-to-actor dedupe。 |

### 16.2.4 最小命名约束

建议后面代码里也保持这个命名边界：

- 所有 envelope 字段统一叫 `request_id` / `event_id`
- 所有 query 往返字段统一叫 `query_id`
- 所有 host coordination 往返字段统一叫 `tool_request_id`
- 不要把 `TurnId`、`ConversationMessageId` 这类业务对象 ID 塞进 transport envelope 上冒充通用 request id

### 16.3 MailboxWrite 草图

```rust
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum SessionMailboxWrite {
    Data(DataMailboxItem),
    Control(ControlMailboxItem),
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum DataMailboxItem {
    UserMessage(UserMessageIngress),
    ActorMessage(ActorMessageIngress),
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UserMessageIngress {
    pub conversation_message_id: ConversationMessageId,
    pub workspace_binding_version: u64,
    pub chat_message: ChatMessage,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ActorMessageIngress {
    pub actor_message_id: ActorMessageId,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ControlMailboxItem {
    CancelTurn { reason: CancelReason },
    ResumePending,
    UpdateWorkspaceBinding { binding: WorkspaceBindingSnapshot },
    Shutdown { reason: ShutdownReason },
    Maintenance(MaintenanceCommand),
}
```

### 16.3.1 MailboxWrite 的幂等语义

为了让这套协议后面能安全落到真实 transport，建议先把幂等规则写清楚：

| 请求类型 | 幂等键建议 | 重复投递时的语义 |
|---|---|---|
| `UserMessageIngress` | `conversation_message_id` | 不应重复入 data mailbox。 |
| `ActorMessageIngress` | `actor_message_id` | 不应重复入 data mailbox。 |
| `CancelTurn` | `request_id` 或显式 command id | 重复 cancel 可以折叠。 |
| `ResolveHostCoordination` | `tool_request_id` | 同一个 tool/request 往返只能 resolve 一次。 |
| `QuerySessionView` | `query_id` | 允许重复 query，但结果应可相关联。 |

最小建议：

- `Data Path` 写入必须做去重
- `Control Path` 可以允许重复，但最好能按 command id 折叠
- query 不要求去重，但必须可用 `query_id` 关联响应

### 16.4 SessionEvent 草图
...
pub enum SessionEvent {
    ...
    InteractiveOutput(InteractiveOutputEvent),
    HostCoordinationRequested(HostCoordinationRequest),
    SessionViewResult(SessionViewResult),
    ControlRejected(ControlRejectedEvent),
    RuntimeCrashed(RuntimeCrashEvent),
}
```

### 16.5 TurnLifecycle 草图
...
pub enum TurnLifecycleEvent {
    ...
    Yielded(TurnYieldedEvent),
    Completed(TurnCompletedEvent),
    Failed(TurnFailedEvent),
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TurnCompletedEvent {
    pub turn_id: TurnId,
    pub consumed_message_ids: Vec<ConversationMessageId>,
    pub assistant_messages: Vec<ChatMessage>,
    pub usage: Option<TokenUsageSnapshot>,
    pub compaction: Option<CompactionSummary>,
    pub final_status: TurnFinalStatus,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TurnFailedEvent {
    pub turn_id: TurnId,
    pub failed_phase: FailedPhase,
    pub failed_operation_kind: FailedOperationKind,
    pub retryable: bool,
...
### 16.5.1 TurnLifecycle 和 transcript 的关系

这里再补一条很重要的约束：

- `TurnLifecycleEvent` 是实时事件流
- session transcript 是 durable read model
- 二者不应该被混成一个东西

推荐语义：

- `Started / Progress / Yielded`
  - 优先服务实时 UI 和 actor orchestration
  - 不要求每一条都映射成 transcript entry
- `Completed / Failed`
  - 会驱动 transcript append、usage 结算、delivery state 更新
- Web 历史读取
  - 读 transcript / detail view
  - 不直接靠重放全部实时 event 来拼出来

### 16.6 HostCoordination 草图

```rust
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HostCoordinationRequest {
    pub tool_request_id: ToolRequestId,
    pub kind: HostCoordinationKind,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum HostCoordinationKind {
    SpawnSubagent(SpawnSubagentRequest),
    StartBackgroundAgent(StartBackgroundAgentRequest),
    DeliverBackgroundResult(DeliverBackgroundResultRequest),
    CronCreate(CronCreateRequestView),
    CronUpdate(CronUpdateRequestView),
    CronDelete { task_id: CronTaskId },
    CronList,
    SnapshotSave(SnapshotSaveRequestView),
    SnapshotLoad { snapshot_id: SnapshotId },
    SnapshotList,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HostCoordinationResolution {
    pub tool_request_id: ToolRequestId,
    pub result: HostCoordinationResult,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum HostCoordinationResult {
    SpawnSubagent(Result<SpawnSubagentResolved, CoordinationError>),
    StartBackgroundAgent(Result<StartBackgroundAgentResolved, CoordinationError>),
    DeliverBackgroundResult(Result<DeliverBackgroundResultResolved, CoordinationError>),
    CronCreate(Result<CronTaskView, CoordinationError>),
    CronUpdate(Result<CronTaskView, CoordinationError>),
    CronDelete(Result<CronDeleteView, CoordinationError>),
    CronList(Result<Vec<CronTaskView>, CoordinationError>),
    SnapshotSave(Result<SnapshotSavedView, CoordinationError>),
    SnapshotLoad(Result<SnapshotLoadedView, CoordinationError>),
    SnapshotList(Result<Vec<SnapshotListItem>, CoordinationError>),
}
```

### 16.6.1 HostCoordination 的错误草图
...
- `SpawnSubagent`
  - 常见失败是 `CapacityExceeded`
- `StartBackgroundAgent`
  - 常见失败是 `Conflict` 或 `InternalError`
- `Cron*`
  - 常见失败是 `InvalidRequest`、`NotFound`
- `Snapshot*`
  - 常见失败是 `NotFound`、`Conflict`
...
### 16.7.1 SessionViewQueryKind 的字段建议
...
这样可以直接对应当前项目里已经存在的几类需求：

- transcript skeleton 分页
- transcript detail 展开
- 当前运行中的 progress / live snapshot 查询

### 16.7.2 QueryError 草图
...
### 16.8 InteractiveOutput 草图

```rust
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum InteractiveOutputEvent {
    ShowOptions(ShowOptionsView),
    ProgressCard(ProgressCardView),
    TranscriptAppended(TranscriptAppendNotice),
    ToolDetailAvailable(ToolDetailAvailableNotice),
}
```

这里的设计意图是：

- `ShowOptions`、Web 特殊卡片、append 通知都走一类 event
- 但 transcript 的实际内容读取仍然走 `QuerySessionView`

### 16.8.1 InteractiveOutput 和 TurnLifecycle 的边界

建议也把这层边界写清楚：

- `TurnLifecycleEvent::Progress`
  - 更偏 actor/runtime 生命周期
- `InteractiveOutputEvent::ProgressCard`
  - 更偏 channel/UI 渲染产物

也就是说：

- 一个 runtime progress 可以触发零个、一个或多个交互输出
- channel 适配层可以按 UI 能力把 lifecycle event 转成不同渲染
- 不要强迫所有 channel 都消费完全相同的 UI event 形态

### 16.9 当前代码到协议草图的映射建议

为了让 `P1` 后面能直接开工，这里把当前代码中的主要对象先映射到新协议。

| 当前代码/概念 | 未来协议对应 |
|---|---|
| `SessionActorRef::tell_user_message` | `SessionRequest::MailboxWrite(Data::UserMessage)` |
| `SessionActorRef::tell_actor_message` | `SessionRequest::MailboxWrite(Data::ActorMessage)` |
| session cancel / yield request | `SessionRequest::MailboxWrite(Control::CancelTurn)` |
| server 侧直接读 transcript list/detail | `SessionRequest::QuerySessionView` |
| server/subagent/cron/snapshot 直接调用 manager | `SessionEvent::HostCoordinationRequested -> SessionRequest::ResolveHostCoordination` |
| 当前 progress fan-out | `TurnLifecycleEvent::Progress` + `InteractiveOutputEvent::ProgressCard` |
| 当前 Web transcript append push | `InteractiveOutputEvent::TranscriptAppended` |

这张映射表的意义是：

- `P1` 不要求立刻删旧代码
- 但任何旧路径都应该能找到一条明确的新协议对应
- 这样 `P2/P3/P4` 才能逐条替换而不是整体推翻

### 16.9 这一版草图的落地建议

`P1` 阶段建议先只做三件事：

1. 在代码里定义这些 `enum/struct`
2. 先把现有 direct method / manager 调用映射到这些类型
3. 先在同进程内复用这套协议，不急着立刻切 UDS/JSON-RPC
...
而不用一上来就承担跨进程 transport 改造的全部风险。

### 16.10 P1 的第一批代码落点建议

如果下一轮就开始改代码，建议先从这些位置下手：

1. 新建一个协议模块
   - 例如 `agent_host/src/session_protocol.rs`
2. 先定义纯数据类型
   - `SessionRequest`
   - `SessionEvent`
   - `HostCoordinationKind`
   - `SessionViewQueryKind`
3. 给现有 direct actor 调用补一层适配函数
   - 先“长得像协议”
   - 暂时还在同进程里调用
4. 先接 `QuerySessionView`
   - 因为它对现有 runtime 侵入最小
5. 再接 `HostCoordinationRequested`
   - 因为它能显著减轻 `server.rs` 的硬编码 orchestration
